...
export const normalizeEntrypointArgs = (
  args: (
    | (DirectiveOrSupplier<any> | DirectiveOrSupplier<any>[])[]
    | PathOptions<any>
  )[],
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (Array.isArray(current) && current.length !== 0) {
      let directives: DirectiveOrSupplier<any>[] = Array.isArray(current)
        ? current.flatMap((d) => d)
        : current;
      ...
      const options =
        nextArg && !Array.isArray(nextArg)
          ? (nextArg as PathOptions<any>)
          : undefined;
...
): Required<SVGOptions<T>> => {
  if (typeof className === "object" && className !== null) {
    options = className;
  }

  const resolvedDefs: UserDefinedResources = {};
  for (const [key, value] of Object.entries(options?.defs ?? {}).filter(
...
export const applyDirectiveModifiers = (
...
export const formatSVGElementID = (
  svgID: string,
  resourceName: string,
): string => {
