...
export const parseVarargs = (
  args: (CSSColorLike | number)[],
): GradientStop[] => {
  ...
  for (const [index, colorOrPercentage] of Object.entries(args) as unknown as [
    number,
    CSSColorLike | number,
  ][]) {
    if (typeof colorOrPercentage === "number") {
      currentStop[1] =
        colorOrPercentage < 1 ? colorOrPercentage * 100 : colorOrPercentage;
    } else {
  ...
  for (let i = 1; i < tuples.length - 1; i++) {
    ...
    if (current[1] < 0) {
      ...
      current[1] =
        (nextStopPercentage - latestStopPercentage) * 0.5 +
        latestStopPercentage;
    }
...
export class LinearGradient implements InternalResource, ReferencableResource {
  ...
  constructor(...stops: (CSSColorLike | number)[]) {
