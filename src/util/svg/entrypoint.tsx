...
const SVG0 = <T extends UserDefinedResources>(
  className?: string | SVGOptions<T>,
  options?: SVGOptions<T>,
): SVGEntrypoint<T> => {
  //Resolve what param the options are in if present
  const normalizedSVGOptions = normalizeSVGOptions(options, className);
  ...
  return (...args) => {
    ...
    const bounds = getBounds(
      resolvedPaths.flatMap((path) =>
        path.directives.flatMap((dir) => dir.getPoints()),
      ),
    );
    ...
    const predefinedResources = {
      ...normalizedSVGOptions.defs,
      ...computedDefs,
    };

    const renderedDefs = appendDefs(
      normalizedSVGOptions.defs,
      predefinedResources,
      svgId,
    );
    ...
    return (
      <svg
        id={`svg-${svgId}`}
        viewBox={computeNormalizedViewBox(bounds)}
        xmlns="http://www.w3.org/2000/svg"
        {...normalizedSVGOptions.htmlAttributes}
      >
        {renderedDefs}
        {resolvedPaths.map((path, idx) =>
          path.toJSXElement(svgId, predefinedResources),
        )}
...
const appendDefs = <T extends UserDefinedResources>(
  internalRessources: T,
  defs: T & ComputedResources,
  svgId: string,
): JSX.Element | null => {
  if (!defs) return null;
  ...
  return (
    <defs>
      {Object.values(internalRessources).map((def) =>
        def.toJSXElement(svgId, defs),
      )}
    </defs>
  );
};
  ...
  implements InternalResource, ReferencableResource
{
  ...
  private static Vec2Directive<
    T extends PredefinedResources = GuaranteedResources,
  >(symbol: DirectiveSymbol, vec: vec2<number>): DrawDirective<T> {
  ...
  private resolveReferencedDefs = <
    T extends PredefinedResources = GuaranteedResources,
  >(
    svgID: string,
    defs: T,
    attributes: PathOptions<T>["htmlAttributes"],
