...
export interface PathOptions<
  T extends PredefinedResources = GuaranteedResources,
> {
...
export class DDEndOfPath<
  T extends PredefinedResources = GuaranteedResources,
> implements DrawDirective<T> {
  toPathString(): string {
    return _DirectiveSymbols.End;
  }
  getPoints(): vec2<number>[] {
    return [];
  }
  ...
  getMirroredOnX(): DDEndOfPath<T> {
    return new DDEndOfPath();
  }
  getMirroredOnY(): DDEndOfPath<T> {
    return new DDEndOfPath();
  }
  getMirroredCustom(
    axisOffset: vec2<number>,
...
export class DrawDirectiveCurve<
  T extends PredefinedResources = GuaranteedResources,
> implements DrawDirective<T> {
  constructor(
  ...
  toPathString(): string {
    return `${_DirectiveSymbols.CurveTo} ${this.x1} ${this.y1}, ${this.x2} ${this.y2}, ${this.x} ${this.y}`;
  }
  getPoints(): vec2<number>[] {
    return [
      [this.x1, this.y1],
  ...
  getSymbol(): DirectiveSymbol {
    return _DirectiveSymbols.CurveTo;
  }
  getMirroredOnX(): DrawDirectiveCurve<T> {
  ...
  getMirroredOnY(): DrawDirectiveCurve<T> {
    return new DrawDirectiveCurve(
      -this.x1,
      this.y1,
      -this.x2,
...
> implements DrawDirective<T> {
  ...
  getSymbol(): DirectiveSymbol {
    return _DirectiveSymbols.ArcTo;
  }
  getMirroredOnX(): DrawDirectiveArc<T> {
  ...
  getMirroredOnY(): DrawDirectiveArc<T> {
    return new DrawDirectiveArc(
      this.rx,
      this.ry,
      -this.rotation,
...
export class DrawDirectiveVec2<
  T extends PredefinedResources = GuaranteedResources,
> implements DrawDirective<T> {
  constructor(
  ...
  getMirroredCustom(
    ...
    return new DrawDirectiveVec2(
      this.symbol,
      mirrorCustomVec2(this.point, axisOffset, angleRad),
    );
  }
  applyConstantOffset(offset: vec2<number>): DrawDirectiveVec2<T> {
    return new DrawDirectiveVec2(this.symbol, [
      this.point[0] + offset[0],
      this.point[1] + offset[1],
    ]);
  }
}

export type DrawDirectiveSupplier<
  T extends PredefinedResources = GuaranteedResources,
> = DrawDirective<T>[] | ((resources: T) => DrawDirective<T>[]);
...
export interface ReferencableResource extends Resource {
...
export type PathModifier<T extends PredefinedResources = GuaranteedResources> =
  (existingDirectives: DrawDirective<T>[]) => DrawDirective<T>[];
...
export type DirectiveOrSupplier<
  T extends PredefinedResources = GuaranteedResources,
> = SelfOrSupplier<DrawDirective<T>, T>;
export type OptionsPathTuple<
  T extends PredefinedResources = GuaranteedResources,
> = [DirectiveOrSupplier<T>[], PathOptions<T>?];
...
export type FlattenedArgs<T extends readonly PredefinedResources[]> =
  T extends readonly [
    infer First extends PredefinedResources,
    ...infer Rest extends readonly PredefinedResources[],
  ]
    ? [
        DirectiveOrSupplier<First>[],
        PathOptions<First>?,
        ...FlattenedArgs<Rest>,
      ]
    : [];
...
export const normalizeVector2 = (vec: vec2<number>): vec2<number> => {
  const mag = Math.sqrt(
    Number(vec[0]) * Number(vec[0]) + Number(vec[1]) * Number(vec[1]),
  );
  if (mag === 0) {
    return [0, 0];
  }
...
export const normalizeVector3 = (vec: vec3<number>): vec3<number> => {
  const mag = Math.sqrt(
    Number(vec[0]) * Number(vec[0]) +
      Number(vec[1]) * Number(vec[1]) +
      Number(vec[2]) * Number(vec[2]),
  );
  if (mag === 0) {
    return [0, 0, 0];
  }
