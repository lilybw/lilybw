...
const computeRayStyle = (index: number, settings: SunRaysProps): JSX.CSSProperties => {
    settings.mvmtFrequencyMultiplier = settings.mvmtFrequencyMultiplier ?? 1;
    settings.numRays = settings.numRays ?? 12;

    const rayMovementFrequency = (Math.random() - 0.5) * 2 * settings.mvmtFrequencyMultiplier; // in Hz
    ...
    let rayColor = `white`;
    switch (index % 4) {
        case 0: rayColor = `hsl(200, calc(50% * var(--bg-saturation-multiplier)), calc(80% * var(--bg-lightness-multiplier)))`; break;
        case 1: rayColor = `hsl(250, calc(50% * var(--bg-saturation-multiplier)), calc(80% * var(--bg-lightness-multiplier)))`; break;
        case 3: rayColor = `hsl(250, calc(50% * var(--bg-saturation-multiplier)), calc(80% * var(--bg-lightness-multiplier)))`; break;
        default: rayColor = `hsl(0, 0%, 100%)`; break;
    }
